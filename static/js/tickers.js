...
function _applySort(rows) {
    ...
    rest.sort((a, b) => {
        switch (_sortCol) {
            case "ticker":
                return _sortDir * a.ticker.localeCompare(b.ticker);
            case "price": {
            ...
            case "chg": {
                const av =
                    a.pct_change ?? (_sortDir > 0 ? Infinity : -Infinity);
                const bv =
                    b.pct_change ?? (_sortDir > 0 ? Infinity : -Infinity);
                return _sortDir * (av - bv);
            }
            case "vol": {
...
function _renderTable(rows) {
    ...
    // Update count badge
    if (_countEl) {
        _countEl.textContent = rows.length
    ...
    _rowsEl
        ...
        .forEach((el) => existing.set(el.dataset.row, el));
    ...
    const needsReorder = children.some(
        (el, i) => el.dataset.row !== ordered[i],
    );
    if (needsReorder) {
        ordered.forEach((sym) => {
    ...
    if (_sortCol === "price") {
        const firstAbove20 = sorted.find(
            (r) => !r.mentioned && r.price != null && r.price > 20,
        );
        const hasBelow20 = sorted.some(
            (r) => !r.mentioned && (r.price == null || r.price <= 20),
        );
        if (firstAbove20 && hasBelow20) {
            const aboveEl = _rowsEl.querySelector(
                `[data-row="${firstAbove20.ticker}"]`,
            );
            if (aboveEl) {
    ...
    for (const row of rows) {
        if (
            row.price != null &&
            _prevPrices[row.ticker] !== undefined &&
            _prevPrices[row.ticker] !== row.price
        ) {
            const priceEl = _rowsEl.querySelector(
                `[data-price="${row.ticker}"]`,
            );
            if (priceEl) {
...
function _rowHTML(row) {
    ...
    const mentionCls = row.mention_burst
        ? " mentions-burst"
        : (row.mention_window ?? 0) > 1
          ? " mentions-active"
          : "";
    const mentionTxt =
        (row.mention_window ?? 0) > 1 ? `${row.mention_window}` : "";
