...
6. **Merge `replay-tool` into `master-mac`** (5cc38ff, 22845f0, 765ca8a) with `ai_watch_admit_arm_gates: true` and `ai_watch_movers_min_rvol: 1.0`. This applies the $20–$100 band to every source, including research (INFQ/CLF hole).
7. **Book server with runway ranking**: supply = Movers + Trending + Research only. One ranked queue: `seat_priority = runway_score + closeness_to_−50`, where runway_score uses day change %, used daily range, EMA9 slope, room below HOD / 30-min swing high (near-HOD scores lower), a morning bump (first 90 min), a small movers bump, and **volume pace** (`rvol_pace_sip`, SIP volume so far vs the stock's 20-day normal; Claude's runway_target_study: pace >= 1.64 took runway from 4% to 16%, z +13.8, both halves). Use pace as a ranking input only, not a gate (`ai_watch_min_rvol_pace` stays 0); before ~09:46, when SIP pace isn't served yet, rank without it. Report replay results split at pace 1.64. Refill seats from the top; short backup queue; start Finnhub/bar warm-up as soon as a source names a ticker. It REPLACES the existing intake/seat paths (momentum/Discord/soft-seed lanes as intake, rising-heat/dead-unknown/heating-rvol supply lanes, immortal src=book rows, duplicate age knobs, engine push-max juggling): when the knob is live those paths are bypassed, and list each retired setting. Put it behind a knob with a **shadow mode** (builds its own book, logs would-have-done, no orders) and default it to shadow unless it clearly beats the current book on the replay.
