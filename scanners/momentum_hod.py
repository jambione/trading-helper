...
class MomentumHodScanner(BaseScanner):
    scanner_type = ScannerType.MOMENTUM_HOD
    ...
    def _evaluate(self, ticker: dict, float_cache: dict[str, Optional[float]]) -> Optional[ScannerAlert]:
        ...
        if high <= 0:
            return None
        if (high - price) / high > HOD_PROXIMITY:
            return None
        if float_m is not None and float_m > FLOAT_MAX_M:
            return None

        return self._build_alert(ticker, float_cache, f"{high:.2f}")
